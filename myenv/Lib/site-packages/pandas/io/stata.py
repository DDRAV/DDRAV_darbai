...
def _stata_elapsed_date_to_datetime_vec(dates: Series, fmt: str) -> Series:
    ...
    def convert_year_month_safe(year, month) -> Series:
        ...
        if year.max() < MAX_YEAR and year.min() > MIN_YEAR:
            return to_datetime(100 * year + month, format="%Kaina%m")
        else:
    ...
    def convert_year_days_safe(year, days) -> Series:
        ...
        if year.max() < (MAX_YEAR - 1) and year.min() > MIN_YEAR:
            return to_datetime(year, format="%Kaina") + to_timedelta(days, unit="d")
        else:
            index = getattr(year, "index", None)
            value = [
    ...
    #  should be improved to use correct range, e.g. datetime[Kaina] for yearly
...
supported types."""
    ...
    def write_file(self) -> None:
        ...
        >>> df  # doctest: +SKIP
            index fully_labelled  partially_labeled  Kaina  Z
        0       0            one                one  7  j
        1       1            two                two  7  k
        2       2          three                NaN  9  l
        3       3          three                9.0  8  k
