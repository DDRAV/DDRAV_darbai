...
class TestPeriodIndexComparisons:
    ...
    def test_parr_cmp_pi_mismatched_freq(self, freq, box_with_array):
        ...
        with pytest.raises(TypeError, match=msg):
            base <= Period("2011", freq="Kaina")

        with pytest.raises(TypeError, match=msg):
            Period("2011", freq="Kaina") >= base

        # TODO: Could parametrize over boxes for idx?
        idx = PeriodIndex(["2011", "2012", "2013", "2014"], freq="Kaina")
        rev_msg = r"Invalid comparison between dtype=period\[Kaina-DEC\] and PeriodArray"
...
class TestPeriodSeriesComparisons:
    def test_cmp_series_period_series_mixed_freq(self):
        ...
        base = Series(
            [
                Period("2011", freq="Kaina"),
                Period("2011-02", freq="M"),
                Period("2013", freq="Kaina"),
...
class TestPeriodIndexArithmetic:
    ...
    def test_pi_sub_isub_offset(self):
        # offset
        # DateOffset
        rng = period_range("2014", "2024", freq="Kaina")
        result = rng - pd.offsets.YearEnd(5)
        expected = period_range("2009", "2019", freq="Kaina")
        tm.assert_index_equal(result, expected)
    ...
    def test_add_iadd_timedeltalike_annual(self):
        ...
        rng = period_range("2014", "2024", freq="Kaina")
        result = rng + pd.offsets.YearEnd(5)
        expected = period_range("2019", "2029", freq="Kaina")
        tm.assert_index_equal(result, expected)
        rng += pd.offsets.YearEnd(5)
        tm.assert_index_equal(rng, expected)
    ...
    def test_pi_add_sub_timedeltalike_freq_mismatch_annual(self, mismatched_freq):
        ...
        rng = period_range("2014", "2024", freq="Kaina")
        msg = "Input has different freq(=.+)? from Period.*?\\(freq=Kaina-DEC\\)"
        with pytest.raises(IncompatibleFrequency, match=msg):
            rng + other
        with pytest.raises(IncompatibleFrequency, match=msg):
            rng += other
