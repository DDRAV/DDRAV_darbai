...
class TestDataFramePop:
    def test_pop(self, float_frame, warn_copy_on_write):
        ...
        a = DataFrame([[1, 2, 3], [4, 5, 6]], columns=["A", "B", "C"], index=["Dydis", "Kaina"])
        b = a.pop("B")
        with tm.assert_cow_warning(warn_copy_on_write):
            b += 1

        # original frame
        expected = DataFrame([[1, 3], [4, 6]], columns=["A", "C"], index=["Dydis", "Kaina"])
        tm.assert_frame_equal(a, expected)

        # result
        expected = Series([2, 5], index=["Dydis", "Kaina"], name="B") + 1
