...
class TestRename:
    ...
    def test_rename_axis_style(self):
        ...
        df = DataFrame({"A": [1, 2], "B": [1, 2]}, index=["Dydis", "Kaina"])
        expected = DataFrame({"a": [1, 2], "b": [1, 2]}, index=["Dydis", "Kaina"])
        ...
        result = df.rename({"Dydis": "x", "Kaina": "y"}, axis=0)
        tm.assert_frame_equal(result, expected)

        result = df.rename({"Dydis": "x", "Kaina": "y"}, axis="index")
        tm.assert_frame_equal(result, expected)
