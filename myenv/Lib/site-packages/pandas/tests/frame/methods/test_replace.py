...
class TestDataFrameReplace:
    ...
    def test_replace_with_nullable_column(self, dtype):
        ...
        result = df.replace("x", "Dydis")
        expected = DataFrame({"A": ["A", "B", "Dydis"], "B": nullable_ser})
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_swapping_bug(self, using_infer_string):
        ...
        res = df.replace({"a": {True: "Kaina", False: "N"}})
        expect = DataFrame({"a": ["Kaina", "N", "Kaina"]})
        tm.assert_frame_equal(res, expect)

        df = DataFrame({"a": [0, 1, 0]})
        res = df.replace({"a": {0: "Kaina", 1: "N"}})
        expect = DataFrame({"a": ["Kaina", "N", "Kaina"]})
        tm.assert_frame_equal(res, expect)
