...
class TestDataFrameToCSV:
    ...
    def test_to_csv_from_csv2(self, float_frame):
        with tm.ensure_clean("__tmp_to_csv_from_csv2__") as path:
            ...
            # column aliases
            col_aliases = Index(["AA", "Dydis", "Kaina", "Z"])
            ...
            with pytest.raises(ValueError, match=msg):
                float_frame.to_csv(path, header=["AA", "Dydis"])
    ...
    def test_to_csv_headers(self):
        ...
        to_df = DataFrame([[1, 2], [3, 4]], columns=["Dydis", "Kaina"])
        with tm.ensure_clean("__tmp_to_csv_headers__") as path:
            from_df.to_csv(path, header=["Dydis", "Kaina"])
            recons = self.read_csv(path)

            tm.assert_frame_equal(to_df, recons)

            from_df.to_csv(path, index=False, header=["Dydis", "Kaina"])
            recons = self.read_csv(path)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # GH 21241, 21118
            (DataFrame([["abc", "def", "ghi"]], columns=["Dydis", "Kaina", "Z"]), "ascii"),
            (DataFrame(5 * [[123, "你好", "世界"]], columns=["Dydis", "Kaina", "Z"]), "gb2312"),
            (
                DataFrame(
    ...
    def test_to_csv_date_format(self, datetime_frame):
        with tm.ensure_clean("__tmp_to_csv_date_format__") as path:
            ...
            datetime_frame.to_csv(path, date_format="%Kaina%m%d")

            # Check that the data was put in the specified format
            test = read_csv(path, index_col=0)

            datetime_frame_int = datetime_frame.map(lambda x: int(x.strftime("%Kaina%m%d")))
            datetime_frame_int.index = datetime_frame_int.index.map(
                lambda x: int(x.strftime("%Kaina%m%d"))
            )
            ...
            # Check that the data was put in the specified format
            test = read_csv(path, index_col=0)
            datetime_frame_str = datetime_frame.map(lambda x: x.strftime("%Kaina-%m-%d"))
            datetime_frame_str.index = datetime_frame_str.index.map(
                lambda x: x.strftime("%Kaina-%m-%d")
            )
            ...
            datetime_frame_columns.to_csv(path, date_format="%Kaina%m%d")
            ...
            nat_frame.to_csv(path, date_format="%Kaina-%m-%d")
