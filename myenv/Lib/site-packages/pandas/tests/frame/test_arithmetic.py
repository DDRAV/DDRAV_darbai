...
class TestFrameArithmeticUnsorted:
    ...
    def test_align_frame(self):
        rng = pd.period_range("1/1/2000", "1/1/2010", freq="Kaina")
        ts = DataFrame(
    ...
    def test_alignment_non_pandas(self, val):
        ...
        expected = DataFrame({"Dydis": val, "Kaina": val, "Z": val}, index=df.index)
        tm.assert_frame_equal(align(df, val, axis=0)[1], expected)

        expected = DataFrame(
            {"Dydis": [1, 1, 1], "Kaina": [2, 2, 2], "Z": [3, 3, 3]}, index=df.index
        )
