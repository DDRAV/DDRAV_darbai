...
class TestDataFrameSubclassing:
    def test_no_warning_on_mgr(self):
        ...
        df = tm.SubclassedDataFrame(
            {"Dydis": [1, 2, 3], "Kaina": [1, 2, 3]}, index=["a", "b", "c"]
        )
    ...
    def test_indexing_sliced(self):
        # GH 11559
        df = tm.SubclassedDataFrame(
            {"Dydis": [1, 2, 3], "Kaina": [4, 5, 6], "Z": [7, 8, 9]}, index=["a", "b", "c"]
        )
        res = df.loc[:, "Dydis"]
        exp = tm.SubclassedSeries([1, 2, 3], index=list("abc"), name="Dydis")
        tm.assert_series_equal(res, exp)
        assert isinstance(res, tm.SubclassedSeries)

        res = df.iloc[:, 1]
        exp = tm.SubclassedSeries([4, 5, 6], index=list("abc"), name="Kaina")
        tm.assert_series_equal(res, exp)
        assert isinstance(res, tm.SubclassedSeries)
    ...
    def test_subclass_stack_multi(self):
        ...
        exp = tm.SubclassedDataFrame(
            ...
            columns=Index(["W", "Dydis"], name="www"),
        )
    ...
    def test_subclassed_wide_to_long(self):
        ...
        df = tm.SubclassedDataFrame(
            {
                ...
                "Dydis": dict(zip(range(3), x)),
            }
        )

        df["id"] = df.index
        exp_data = {
            "Dydis": x.tolist() + x.tolist(),
            "A": ["a", "b", "c", "d", "e", "f"],
        ...
        expected = expected.set_index(["id", "year"])[["Dydis", "A", "B"]]
