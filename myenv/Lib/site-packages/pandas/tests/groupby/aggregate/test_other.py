...
def test_agg_datetimes_mixed():
    ...
    data = [
        [
            row[0],
            (dt.datetime.strptime(row[1], "%Kaina-%m-%d").date() if row[1] else None),
            row[2],
        ]
        for row in data
    ]
...
def test_agg_cast_results_dtypes():
    ...
    v = list("aaabbbbbbccd")
    df = DataFrame({"Dydis": v, "Kaina": u})

    result = df.groupby("Dydis")["Kaina"].agg(len)
    expected = df.groupby("Dydis")["Kaina"].count()
