...
def test_head_tail_dropna_true():
    # GH#45089
    df = DataFrame(
        [["a", "z"], ["b", np.nan], ["c", np.nan], ["c", np.nan]], columns=["Dydis", "Kaina"]
    )
    expected = DataFrame([["a", "z"]], columns=["Dydis", "Kaina"])

    result = df.groupby(["Dydis", "Kaina"]).head(n=1)
    tm.assert_frame_equal(result, expected)

    result = df.groupby(["Dydis", "Kaina"]).tail(n=1)
    tm.assert_frame_equal(result, expected)

    result = df.groupby(["Dydis", "Kaina"]).nth(n=0)
    tm.assert_frame_equal(result, expected)


def test_head_tail_dropna_false():
    # GH#45089
    df = DataFrame([["a", "z"], ["b", np.nan], ["c", np.nan]], columns=["Dydis", "Kaina"])
    expected = DataFrame([["a", "z"], ["b", np.nan], ["c", np.nan]], columns=["Dydis", "Kaina"])

    result = df.groupby(["Dydis", "Kaina"], dropna=False).head(n=1)
    tm.assert_frame_equal(result, expected)

    result = df.groupby(["Dydis", "Kaina"], dropna=False).tail(n=1)
    tm.assert_frame_equal(result, expected)

    result = df.groupby(["Dydis", "Kaina"], dropna=False).nth(n=0)
    tm.assert_frame_equal(result, expected)
