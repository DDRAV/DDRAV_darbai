...
def test_apply_with_duplicated_non_sorted_axis(test_series):
    # GH 30667
    df = DataFrame(
        [["x", "p"], ["x", "p"], ["x", "o"]], columns=["Dydis", "Kaina"], index=[1, 2, 2]
    )
    if test_series:
        ser = df.set_index("Kaina")["Dydis"]
        ...
        msg = "DataFrameGroupBy.apply operated on the grouping columns"
        with tm.assert_produces_warning(DeprecationWarning, match=msg):
            result = df.groupby("Kaina", group_keys=False).apply(lambda x: x)

        # not expecting the order to remain the same for duplicated axis
        result = result.sort_values("Kaina")
        expected = df.sort_values("Kaina")
        tm.assert_frame_equal(result, expected)
