...
def test_get_values_for_csv():
    ...
    result = index._get_values_for_csv(date_format="%m-%Kaina-%d")
    tm.assert_numpy_array_equal(result, expected)
    ...
    result = index._get_values_for_csv(na_rep="NaT", date_format="%Kaina-%m-%d %H:%M:%S.%f")
    expected = np.array(
...
class TestFormat:
    def test_format(self):
        ...
        expected = [f"{x:%Kaina-%m-%d}" for x in idx]
    ...
    def test_format_date_explicit_date_format(self):
        ...
        msg = "DatetimeIndex.format is deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            formatted = dti.format(date_format="%m-%d-%Kaina", na_rep="UT")
