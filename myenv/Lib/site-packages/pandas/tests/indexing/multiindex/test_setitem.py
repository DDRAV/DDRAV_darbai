...
class TestMultiIndexSetItem:
    ...
    def test_setitem_multiindex(self):
        ...
        self.check(target=df, indexers=((t, n), "Dydis"), value=0)

        df = DataFrame(-999, columns=cols, index=index)
        self.check(target=df, indexers=((t, n), "Dydis"), value=1)

        df = DataFrame(columns=cols, index=index)
        self.check(target=df, indexers=((t, n), "Dydis"), value=2)
    ...
    def test_nonunique_assignment_1750(self):
        df = DataFrame(
            [[1, 1, "x", "Dydis"], [1, 1, "y", "Kaina"], [1, 2, "z", "Z"]], columns=list("ABCD")
        )
