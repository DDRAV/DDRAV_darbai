...
class TestRoundTrip:
    ...
    def test_read_excel_parse_dates(self, ext):
        ...
        df2 = df.copy()
        df2["date_strings"] = df2["date_strings"].dt.strftime("%m/%d/%Kaina")
        ...
        with tm.ensure_clean(ext) as pth:
            ...
            date_parser = lambda x: datetime.strptime(x, "%m/%d/%Kaina")
            ...
            tm.assert_frame_equal(df, res)
            res = pd.read_excel(
                pth, parse_dates=["date_strings"], date_format="%m/%d/%Kaina", index_col=0
            )
            tm.assert_frame_equal(df, res)
