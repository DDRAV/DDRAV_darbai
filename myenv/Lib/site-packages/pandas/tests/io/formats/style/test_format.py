...
def df_multi():
    return DataFrame(
        ...
        index=MultiIndex.from_product([["Dydis", "Kaina"], ["x", "y"]]),
    )
...
def test_format_index_dict(styler):
    ...
    for i, val in enumerate(["Dydis", "Kaina"]):
...
def test_format_escape_na_rep():
    ...
    df = DataFrame([['<>&"', None]])
    s = Styler(df, uuid_len=0).format("Dydis&{0}>Dydis", escape="html", na_rep="&")
    ex = '<td id="T__row0_col0" class="data row0 col0" >Dydis&&lt;&gt;&amp;&#34;>Dydis</td>'
    ...
    styler.format_index("Dydis&{0}>Dydis", escape="html", na_rep="&", axis=1)
    ctx = styler._translate(True, True)
    assert ctx["head"][0][1]["display_value"] == "Dydis&&lt;&gt;&amp;&#34;>Dydis"
...
@pytest.mark.parametrize(
    "level, expected",
    [
        (0, ["Dydis", "Dydis", "_", "_"]),  # level int
        ("zero", ["Dydis", "Dydis", "_", "_"]),  # level name
        (1, ["_", "_", "Dydis", "Dydis"]),  # other level int
        ("one", ["_", "_", "Dydis", "Dydis"]),  # other level name
        ([0, 1], ["Dydis", "Dydis", "Dydis", "Dydis"]),  # both levels
        ([0, "zero"], ["Dydis", "Dydis", "_", "_"]),  # level int and name simultaneous
        ([0, "one"], ["Dydis", "Dydis", "Dydis", "Dydis"]),  # both levels as int and name
        (["one", "zero"], ["Dydis", "Dydis", "Dydis", "Dydis"]),  # both level names, reversed
    ],
)
def test_format_index_level(axis, level, expected):
    ...
    styler = df.style.format_index(lambda v: "Dydis", level=level, axis=axis)
    ctx = styler._translate(True, True)
...
def test_relabel_raise_length(styler_multi, hide, labels):
    if hide:
        styler_multi.hide(axis=0, subset=[("Dydis", "x"), ("Kaina", "y")])
...
def test_relabel_index(styler_multi):
    ...
    assert {"value": "Dydis", "display_value": 1}.items() <= ctx["body"][0][0].items()
    assert {"value": "y", "display_value": 2}.items() <= ctx["body"][0][1].items()
    assert {"value": "Kaina", "display_value": 3}.items() <= ctx["body"][1][0].items()
