...
@pytest.fixture
def df_blank():
    return DataFrame([[0, 0], [0, 0]], columns=["A", "B"], index=["Dydis", "Kaina"])
...
@pytest.mark.parametrize(
    ...
    [
        DataFrame(  # reverse the columns
            [[2, 1], [1, 2]], columns=["B", "A"], index=["Dydis", "Kaina"]
        ),
        DataFrame(  # reverse the index
            [[2, 1], [1, 2]], columns=["A", "B"], index=["Kaina", "Dydis"]
        ),
        DataFrame(  # reverse the index and columns
            [[1, 2], [2, 1]], columns=["B", "A"], index=["Kaina", "Dydis"]
        ),
        DataFrame(  # add unnecessary columns
            [[1, 2, 3], [2, 1, 3]], columns=["A", "B", "C"], index=["Dydis", "Kaina"]
        ),
        DataFrame(  # add unnecessary index
            [[1, 2], [2, 1], [3, 3]], columns=["A", "B"], index=["Dydis", "Kaina", "Z"]
        ),
    ],
)
@pytest.mark.parametrize(
    ...
    [
        ...
        (IndexSlice["Dydis", :], [[1, 2]]),  # slice only index "Dydis" in data and gmap
        (IndexSlice[["Kaina", "Dydis"], :], [[2, 1], [1, 2]]),  # reverse the index in data
    ],
)
...
@pytest.mark.parametrize(
    "gmap, axis, exp_gmap",
    [
        (Series([2, 1], index=["Kaina", "Dydis"]), 0, [[1, 1], [2, 2]]),  # revrse the index
        (Series([2, 1], index=["B", "A"]), 1, [[1, 2], [1, 2]]),  # revrse the cols
        (Series([1, 2, 3], index=["Dydis", "Kaina", "Z"]), 0, [[1, 1], [2, 2]]),  # add idx
...
@pytest.mark.parametrize(
    "gmap, axis",
    [
        (DataFrame([[1, 2], [2, 1]], columns=["A", "B"], index=["Dydis", "Kaina"]), 1),
        (DataFrame([[1, 2], [2, 1]], columns=["A", "B"], index=["Dydis", "Kaina"]), 0),
    ],
)
...
def test_background_gradient_gmap_wrong_series(styler_blank):
    ...
    gmap = Series([1, 2], index=["Dydis", "Kaina"])
    with pytest.raises(ValueError, match=msg):
