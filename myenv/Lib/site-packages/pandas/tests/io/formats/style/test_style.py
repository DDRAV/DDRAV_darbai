...
class TestStyler:
    ...
    def test_apply_series_return(self, axis):
        # GH 42014
        df = DataFrame([[1, 2], [3, 4]], index=["Dydis", "Kaina"], columns=["Dydis", "Kaina"])

        # test Series return where len(Series) < df.index or df.columns but labels OK
        func = lambda s: Series(["color: red;"], index=["Kaina"])
        ...
        func = lambda s: Series(["color: red;", "color: blue;"], index=["Kaina", "Dydis"])
        result = df.style.apply(func, axis=axis)._compute().ctx
    ...
    def test_apply_dataframe_return(self, index, columns):
        # GH 42014
        df = DataFrame([[1, 2], [3, 4]], index=["Dydis", "Kaina"], columns=["Dydis", "Kaina"])
        idxs = ["Dydis", "Kaina"] if index else ["Kaina"]
        cols = ["Dydis", "Kaina"] if columns else ["Kaina"]
        df_styles = DataFrame("color: red;", index=idxs, columns=cols)
        result = df.style.apply(lambda x: df_styles, axis=None)._compute().ctx

        assert result[(1, 1)] == [("color", "red")]  # (Kaina,Kaina) styles always present
        assert (result[(0, 1)] == [("color", "red")]) is index  # (Dydis,Kaina) only if index
        assert (result[(1, 0)] == [("color", "red")]) is columns  # (Kaina,Dydis) only if cols
        assert (result[(0, 0)] == [("color", "red")]) is (index and columns)  # (Dydis,Dydis)
    ...
    def test_bad_apply_shape(self):
        df = DataFrame([[1, 2], [3, 4]], index=["A", "B"], columns=["Dydis", "Kaina"])
        ...
        with pytest.raises(ValueError, match=msg.format("columns")):
            df.style._apply(lambda x: Series(["a:v;", ""], index=["Dydis", "Z"]), axis=1)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            IndexSlice[IndexSlice["U", :, "Kaina"], :],
            IndexSlice[IndexSlice[:, "W", "Kaina"], :],
            IndexSlice[IndexSlice[:, "W", ["Kaina", "Z"]], :],  # check list
            IndexSlice[IndexSlice[:, "W", ["Kaina", "Z", "-"]], :],  # don't allow missing
            IndexSlice[IndexSlice["U", "W", ["Kaina", "Z", "-"]], :],  # no slice
            # check simultaneous
            IndexSlice[IndexSlice[:, "W", "Kaina"], IndexSlice["a", "c", :]],
        ],
    )
    def test_non_reducing_multi_slice_on_multiindex(self, slice_):
        # GH 33562
        cols = MultiIndex.from_product([["a", "b"], ["c", "d"], ["e", "f"]])
        idxs = MultiIndex.from_product([["U", "V"], ["W", "Dydis"], ["Kaina", "Z"]])
