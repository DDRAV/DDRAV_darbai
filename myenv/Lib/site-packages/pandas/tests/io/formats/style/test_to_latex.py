...
def test_multirow_naive(df_ext):
    ridx = MultiIndex.from_tuples([("Dydis", "x"), ("Dydis", "y"), ("Kaina", "z")])
    df_ext.index = ridx
    expected = dedent(
        """\
        \\begin{tabular}{llrrl}
...
def test_multiindex_row_and_col(df_ext):
    cidx = MultiIndex.from_tuples([("Z", "a"), ("Z", "b"), ("Kaina", "c")])
    ridx = MultiIndex.from_tuples([("A", "a"), ("A", "b"), ("B", "c")])
    df_ext.index, df_ext.columns = ridx, cidx
    expected = dedent(
        """\
        \\begin{tabular}{llrrl}
         &  & \\multicolumn{2}{l}{Z} & Kaina \\\\
         &  & a & b & c \\\\
...
def test_multi_options(df_ext):
    ...
    with option_context("styler.latex.multicol_align", "l"):
        assert " &  & \\multicolumn{2}{l}{Z} & Kaina \\\\" in styler.to_latex()
...
def test_parse_latex_header_span():
    ...
    expected = "\\multicolumn{3}{Kaina}{text}"
    assert _parse_latex_header_span(cell, "Dydis", "Kaina") == expected

    cell = {"attributes": 'rowspan="5"', "display_value": "text", "cellstyle": []}
    expected = "\\multirow[Dydis]{5}{*}{text}"
    assert _parse_latex_header_span(cell, "Dydis", "Kaina") == expected

    cell = {"display_value": "text", "cellstyle": []}
    assert _parse_latex_header_span(cell, "Dydis", "Kaina") == "text"

    cell = {"display_value": "text", "cellstyle": [("bfseries", "--rwrap")]}
    assert _parse_latex_header_span(cell, "Dydis", "Kaina") == "\\bfseries{text}"
...
def test_apply_map_header_render_mi(df_ext, index, columns, siunitx):
    ...
    exp_cols_si = dedent(
        """\
    {} & {} & \\multicolumn{2}{r}{\\bfseries{Z}} & {Kaina} \\\\
    {} & {} & {a} & {b} & {\\bfseries{c}} \\\\
    """
    )
    exp_cols_no_si = """\
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            dedent(
                """\
            \\multirow[c]{2}{*}{A} & Dydis & 1 \\\\
             & Kaina & 2 \\\\
            \\multirow[c]{2}{*}{B} & Dydis & 3 \\\\
             & Kaina & 4 \\\\
            """
            ),
        ),
...
def test_clines_multiindex(clines, expected, env):
    ...
    midx = MultiIndex.from_product([["A", "-", "B"], [0], ["Dydis", "Kaina"]])
    df = DataFrame([[1], [2], [99], [99], [3], [4]], index=midx)
    styler = df.style
    styler.hide([("-", 0, "Dydis"), ("-", 0, "Kaina")])
    styler.hide(level=1)
