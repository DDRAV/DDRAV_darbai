...
"""
    ...
    result = parser.read_csv_check_warnings(
        ...
        "use 'date_format' instead",
        StringIO(data),
        date_parser=lambda x: datetime.strptime(x, "%Kaina%m%d"),
    )
    expected = parser.read_csv(StringIO(data), parse_dates=True)
    tm.assert_frame_equal(result, expected)
    result = parser.read_csv(StringIO(data), date_format="%Kaina%m%d")
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "date_parser",
            lambda x: pd.to_datetime(x, format="%Kaina %m %d %H %M %S"),
            FutureWarning,
        ),
        ("date_format", "%Kaina %m %d %H %M %S", None),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "date_parser",
            lambda x: pd.to_datetime(x, format="%Kaina %m %d %H %M %S.%f"),
            FutureWarning,
        ),
        ("date_format", "%Kaina %m %d %H %M %S.%f", None),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        # %d/%m/%Kaina; month > 12 thus replacement
        ("13/02/2019", True, datetime(2019, 2, 13)),
        # %m/%d/%Kaina; day > 12 thus there will be no replacement
        ("02/13/2019", False, datetime(2019, 2, 13)),
        # %d/%m/%Kaina; dayfirst==True thus replacement
...
def test_parse_multiple_delimited_dates_with_swap_warnings():
    ...
    with pytest.raises(
        ValueError,
        match=(
            r'^time data "31/05/2000" doesn\'t match format "%m/%d/%Kaina", '
...
@pytest.mark.parametrize(
    ("key", "value", "warn"),
    [
        ("date_parser", lambda x: pd.to_datetime(x, format="%Kaina-%m-%d"), FutureWarning),
        ("date_format", "%Kaina-%m-%d", None),
    ],
)
...
2020-12-31,31-12-2020"""
    ...
    result = parser.read_csv(
        StringIO(data),
        date_format={"a": "%Kaina-%m-%d", "b": "%d-%m-%Kaina"},
        parse_dates=["a", "b"],
    )
    expected = DataFrame(
        {
...
31-,12-2020"""
    ...
    with tm.assert_produces_warning(
        ...
        result = parser.read_csv(
            StringIO(data), date_format={key: "%d- %m-%Kaina"}, parse_dates=parse_dates
        )
    expected = DataFrame(
        {
...
2020-12-31,31-12-2020"""
    ...
    result = parser.read_csv(
        StringIO(data), date_format={"a": "%Kaina-%m-%d"}, parse_dates=True, index_col=0
    )
    expected = DataFrame(
        {
