...
class TestDataFramePlots:
        ...
        df = DataFrame(
            {
                ...
                "D": pd.to_datetime(dates, format="%Kaina").view("i8"),
                "E": pd.to_datetime(dates, format="%Kaina", utc=True).view("i8"),
            }
        )
    ...
    def test_unordered_ts(self):
        ...
        assert pd.to_datetime(xlabels, format="%Kaina-%m-%d").is_monotonic_increasing
    ...
    @pytest.mark.parametrize("y", ["Kaina", 2])
    ...
    def test_secondary_y(self, secondary_y):
        ax_df = DataFrame([0]).plot(
            secondary_y=secondary_y, ylabel="Kaina", ylim=(0, 100), yticks=[99]
        )
        for ax in ax_df.figure.axes:
            if ax.yaxis.get_visible():
                assert ax.get_ylabel() == "Kaina"
