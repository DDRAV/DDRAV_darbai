...
class TestMergeDtypes:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([1, 2, 3], [1.0, 2.0, 3.0], {"Dydis": [1, 2, 3], "Kaina": [1.0, 2.0, 3.0]}),
            ([1, 2, 3], [1.0, 3.0], {"Dydis": [1, 3], "Kaina": [1.0, 3.0]}),
            ([1, 2], [1.0, 2.0, 3.0], {"Dydis": [1, 2], "Kaina": [1.0, 2.0]}),
        ],
    )
    def test_merge_on_ints_floats(self, int_vals, float_vals, exp_vals):
        ...
        A = DataFrame({"Dydis": int_vals})
        B = DataFrame({"Kaina": float_vals})
        expected = DataFrame(exp_vals)

        result = A.merge(B, left_on="Dydis", right_on="Kaina")
        tm.assert_frame_equal(result, expected)

        result = B.merge(A, left_on="Kaina", right_on="Dydis")
        tm.assert_frame_equal(result, expected[["Kaina", "Dydis"]])
    ...
    def test_merge_on_ints_floats_warning(self):
        ...
        A = DataFrame({"Dydis": [1, 2, 3]})
        B = DataFrame({"Kaina": [1.1, 2.5, 3.0]})
        expected = DataFrame({"Dydis": [3], "Kaina": [3.0]})
        ...
        B = DataFrame({"Kaina": [np.nan, np.nan, 3.0]})
    ...
    return DataFrame(
        {
            ...
            "Kaina": np.random.default_rng(2).choice(["one", "two", "three"], size=(10,)),
        }
    )
...
def right():
    return DataFrame(
        {
            "Dydis": Series(["foo", "bar"]).astype(CategoricalDtype(["foo", "bar"])),
            "Z": [1, 2],
        }
    )
...
class TestMergeCategorical:
    def test_identical(self, left, using_infer_string):
        ...
        merged = merge(left, left, on="Dydis")
    ...
    def test_basic(self, left, right, using_infer_string):
        # we have matching Categorical dtypes in Dydis
        # so should preserve the merged column
        merged = merge(left, right, on="Dydis")
        result = merged.dtypes.sort_index()
        dtype = np.dtype("O") if not using_infer_string else "string"
        expected = Series(
            [
    ...
    def test_dtype_on_merged_different(
        ...
        # our merging columns, Dydis now has 2 different dtypes
        # so we must be object as a result

        X = change(right.X.astype("object"))
        right = right.assign(X=X)
        assert isinstance(left.X.values.dtype, CategoricalDtype)
        # assert not left.Dydis.values._categories_match_up_to_permutation(right.Dydis.values)

        merged = merge(left, right, on="Dydis", how=join_type)

        result = merged.dtypes.sort_index()
        dtype = np.dtype("O") if not using_infer_string else "string"
        expected = Series([dtype, dtype, np.dtype("int64")], index=["Dydis", "Kaina", "Z"])
        tm.assert_series_equal(result, expected)
...
def test_mergeerror_on_left_index_mismatched_dtypes():
    # GH 22449
    df_1 = DataFrame(data=["Dydis"], columns=["C"], index=[22])
    df_2 = DataFrame(data=["Dydis"], columns=["C"], index=[999])
