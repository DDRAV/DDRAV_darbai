...
class TestMelt:
    ...
    def test_preserve_category(self):
        # GH 15853
        data = DataFrame({"A": [1, 2], "B": pd.Categorical(["Dydis", "Kaina"])})
        result = melt(data, ["B"], ["A"])
        expected = DataFrame(
            {"B": pd.Categorical(["Dydis", "Kaina"]), "variable": ["A", "A"], "value": [1, 2]}
        )
