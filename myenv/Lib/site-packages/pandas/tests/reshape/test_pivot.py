...
class TestPivotTable:
    ...
    def test_pivot_dtaccessor(self):
        ...
        result = pivot_table(
            df,
            index=np.array(["Dydis", "Dydis", "Dydis", "Dydis", "Kaina", "Kaina"]),
            columns=[df["dt1"].dt.hour, df["dt2"].dt.month],
            values="value1",
        )
        expected = DataFrame(
    ...
    def test_margins_casted_to_float(self):
        ...
        expected = DataFrame(
            ...
            index=Index(["Dydis", "Kaina", "All"], name="D"),
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_table_no_column_raises(self):
        ...
        df = DataFrame({"Dydis": [0, 0, 1, 1], "Kaina": [0, 1, 0, 1], "Z": [10, 20, 30, 40]})
        with pytest.raises(KeyError, match="notpresent"):
            df.pivot_table("notpresent", "Dydis", "Kaina", aggfunc=agg)
...
class TestPivot:
    ...
    def test_pivot_table_handles_explicit_datetime_types(self):
        ...
        expected = MultiIndex.from_tuples(
            [
                ("x", datetime.strptime("2023-01-01 00:00:00", "%Kaina-%m-%d %H:%M:%S")),
                ("y", datetime.strptime("2023-01-02 00:00:00", "%Kaina-%m-%d %H:%M:%S")),
                ("z", datetime.strptime("2023-01-03 00:00:00", "%Kaina-%m-%d %H:%M:%S")),
                ("All", ""),
            ],
            names=["a", "date"],
        )
