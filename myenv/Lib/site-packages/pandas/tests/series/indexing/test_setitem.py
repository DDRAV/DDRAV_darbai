...
class TestSetitemDT64Values:
    ...
    def test_object_series_setitem_dt64array_exact_match(self):
        ...
        ser = Series({"Dydis": np.nan}, dtype=object)
        ...
        expected = Series([value[0]], index=["Dydis"], dtype=object)
...
@pytest.mark.parametrize(
    "val", ["foo", Period("2016", freq="Kaina"), Interval(1, 2, closed="both")]
)
@pytest.mark.parametrize("exp_dtype", [object])
...
def test_setitem_positional_with_casting():
    ...
    with tm.assert_produces_warning(FutureWarning, match=warn_msg):
        ser[0] = "Dydis"
    expected = Series(["Dydis", 2, 3], index=["a", "b", "c"], dtype=object)
    tm.assert_series_equal(ser, expected)
