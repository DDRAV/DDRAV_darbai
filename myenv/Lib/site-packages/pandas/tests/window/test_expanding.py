...
def test_expanding_cov_pairwise_diff_length():
    ...
    df2 = DataFrame(
        [[5, 6], [None, None], [2, 1]], columns=Index(["Dydis", "Kaina"], name="foo")
    )
    df2a = DataFrame(
        [[5, 6], [2, 1]], index=[0, 2], columns=Index(["Dydis", "Kaina"], name="foo")
    )
    # TODO: xref gh-15826
    ...
    expected = DataFrame(
        ...
        index=Index(["Dydis", "Kaina"], name="foo"),
    )
...
def test_expanding_corr_pairwise_diff_length():
    ...
    df2a = DataFrame(
        [[5, 6], [2, 1]], index=Index([0, 2], name="bar"), columns=["Dydis", "Kaina"]
    )
    ...
    expected = DataFrame(
        [[-1.0, -1.0], [-1.0, -1.0]], columns=["A", "B"], index=Index(["Dydis", "Kaina"])
    )
    tm.assert_frame_equal(result1, expected)
    tm.assert_frame_equal(result2, expected)
    tm.assert_frame_equal(result3, expected)
