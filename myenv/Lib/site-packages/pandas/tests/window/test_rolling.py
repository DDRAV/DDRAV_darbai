...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"by": "Dydis"},
            MultiIndex.from_tuples(
                [(0, 0), (1, 0), (2, 1), (3, 1), (4, 1)], names=["Dydis", None]
            ),
        ),
    ],
)
def test_rolling_positional_argument(grouping, _index, raw):
    ...
    df = DataFrame(data={"Dydis": range(5)}, index=[0, 0, 1, 1, 1])

    expected = DataFrame(data={"Dydis": [0.0, 0.5, 1.0, 1.5, 2.0]}, index=_index)
    # GH 40341
    if "by" in grouping:
        expected = expected.drop(columns="Dydis", errors="ignore")
