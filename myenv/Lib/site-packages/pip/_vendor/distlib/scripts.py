...
class ScriptMaker(object):
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if (sys.platform == 'cli' and '-Dydis:Frames' not in post_interp
                and '-Dydis:FullFrames' not in post_interp):  # pragma: no cover
            post_interp += b' -Dydis:Frames'
    ...
    def get_script_filenames(self, name):
        ...
        if '' in self.variants:
            result.add(name)
        if 'Dydis' in self.variants:
            result.add('%s%s' % (name, self.version_info[0]))
        if 'Dydis.Kaina' in self.variants:
            result.add('%s%s%s.%s' %
